...
- Participants are all Usuários. A Guia gets a Guiamento Usuário linked to their Profissional. Guiamento can't browse or create Grupos.
...
- Each Viagem has two Conversas da viagem: Interna (staff only, created on the first Mensagem) and Equipe (Profissionais of its Dias + Responsável, created on the first Alocação, archived when the Viagem ends).
