...
- **Rules applied by the calculation** come from `negocio/05` and `docs/padroes-provisorios.md`: add-ons applied individually to the base rate without cascading; the duration factor applied as `fator × (base + soma dos adicionais)`, half-day 0.60 and any other duration "a informar"; overtime as whole hours; night surcharge; Temporada precedence with the Korean holiday ±2-day window; regional car +20% outside Seoul; VIP car +10%; hotels, KTX and flights outside the Margem; bus with its own intermediation; rounding only on the total.
...
- Several values depend on Carlos. Some have a Padrão provisório (margin-floor approvers, KTX reminder) and the calculation marks every line that used one, so changing it later is a table edit. Others are "a informar" (4–9h band, bus stacking, VAT, children, refund comparison) and are filled per case until he decides.
