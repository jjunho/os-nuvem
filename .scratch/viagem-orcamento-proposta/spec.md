...
Staff open a **Viagem** at the first contact, whether it comes through an Agência (B2B) or directly (B2C). A **Responsável** and a **Próxima ação** exist from the start, so no lead goes unowned or unanswered. The Viagem's **Etapa** is inferred from what is recorded on it and never set by hand (ADR-0007). The negotiation lives on the Viagem, not in a spreadsheet cell.
...
22. As a salesperson, I want each Viagem to show exactly one Etapa (lead, em orçamento, proposta enviada, em negociação, confirmada, em viagem, concluída, perdida, cancelada, descartada), inferred from what was recorded on it, so that the Pipeline is readable and true.
...
25. As a salesperson, I want a follow-up Próxima ação created 3 days after a Proposta is sent, continuing every 3 days after the third one goes unanswered, with the Viagem marked "sem resposta" and an alert to me and the Admin, so that we fight for every Viagem and only a person declares it perdida.
26. As Carlos, I want a Pipeline view of all open Viagens by Etapa, Responsável and overdue Próxima ação, so that I can see what is at risk. (Its kanban layout is in `.scratch/quadros-e-kanban/spec.md`.)
26a. As a salesperson, I want to record the Cliente's reply (aceitou, pediu mudanças, recusou, ainda pensando), so that a fact that happened outside the system still moves the Etapa.
26b. As a Responsável or Admin, I want a Correção de etapa when a fact was recorded by mistake, with who, when and why, so that errors are fixed without a manual backward move.
...
60. As a salesperson, I want Condições filled from defaults (Sinal 30% and Saldo due 30 days before the trip, editable; Juliano 2026-09-24, see `docs/padroes-provisorios.md`, validity 15 days, cancellation terms, payment methods and bank details) and editable per Proposta, so that terms are always stated but still negotiable.
...
- **Modules:**
  - **Viagens**: Viagem, Contato, Agência, Cliente, roles, Responsável history, Etapa, Próxima ação, Origem, notes, Viagens relacionadas. It records the facts and stores the Etapa inferred from them (ADR-0007; the Inferência de etapa and the Etapa templates are in `.scratch/quadros-e-kanban/spec.md`). Its automatic first reply and follow-ups become Tarefas da etapa.
...
- **Etapa** is an explicit state, not free text, and it is inferred from facts, never set by hand (ADR-0007). Lead on creation; em orçamento when the first Orçamento is created; proposta enviada when a Proposta Envio is recorded; em negociação when the Cliente asks for changes or a new Versão is started after an Envio (em negociação can be skipped); confirmada when an Aceite is recorded. Descartada (from any open Etapa) and cancelada (only from confirmada) are declared with a reason. Perdida is declared only by a person, with a Motivo de perda; the system never sets it. After the third unanswered follow-up, the follow-ups continue every 3 days and the Viagem is marked "sem resposta". There is no backward move: a fact recorded by mistake is undone by a Correção de etapa (Responsável or Admin, with why). Em viagem and concluída belong to the operations spec.
...
- **Vertical tests, one set per part.** Each part below is built and tested as a thin slice through all layers (screen or API → module → database) before the next part starts. A part is done when its vertical tests pass.
  ...
  2. **Etapas and pipeline:** the Etapa inferred from each fact (Orçamento created, Proposta Envio, the Cliente's reply, new Versão after an Envio, Aceite), no way to set it by hand, closing as perdida with a Motivo de perda, the "sem resposta" mark with continuing follow-ups and no automatic perdida, a Correção de etapa, and the pipeline view showing overdue Próximas ações.
...
- **One transversal test, written last,** passes through every step in order, using one realistic B2B case (an Agência requesting 10 pagantes + 2 gratuidades and 12 + 2 at premium) and one B2C case:
  first contact → Viagem with Responsável → first reply → Orçamento built Dia by Dia → two Opções → Ajuste manual with reason → Preço enviado → send (Versão frozen, Etapa proposta enviada, follow-up created) → the Cliente asks for changes (em negociação) → Versão 2 sent (proposta enviada) → Proposta PDF → Cliente accepts one Opção → Viagem confirmada.
