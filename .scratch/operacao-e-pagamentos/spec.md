...
- **The transversal test is written last.** It continues the first spec's transversal case, the B2B Agência with 10 pagantes + 2 gratuidades, through every step:
  ...
  8. Invoice for the Saldo issued, and the Saldo recorded by its due date;
  9. Aviso do dia;
...
- Relevant defaults are in `docs/padroes-provisorios.md`:
  - Sinal 30% and Saldo 30 days before the trip, editable (Juliano, 2026-09-24, to confirm with Carlos), with warnings when the Sinal does not cover committed penalties or when a non-refundable item is about to be issued before the whole Saldo is paid (K918).
...
A full read of `../docs/negocio` found gaps, now carried by the tickets in `issues/`. Decisions taken with Juliano (every value below is an editable default with a sane starting value, ADR-0001):
- **Who handles payments:** Propostas e Orçamentos issues Invoices, records Pagamentos and sees the Situação de pagamento and the Saldo. Faturamento and Admin also do reconciliation, Contas a pagar and the Resultado da viagem. Itinerários, Guiamento and Conteúdo see no payment data. Funcionários' salaries never appear in the app.
- **Sinal and Saldo:** default Sinal 30% and Saldo 30 days before the trip, editable per Proposta (to confirm with Carlos). Carlos's rule K918 applies: the whole Saldo is paid before any non-refundable item is issued, and the Sinal covers the penalties already exposed; going ahead anyway needs a recorded override by Faturamento or Admin (ADR-0001). Story 32 is replaced by this rule.
- **Voucher number:** the Voucher has its own number `AAAAMMDD-N` from the arrival date (K305), recorded for audit: generated by the system, unique, never reused (not even after a cancelamento), kept across versions and never renumbered when the dates change; every issue records who, when and which version. The Código da viagem, created at first contact, stays on every document too.
- **Contacts on the Voucher:** the team phone always; each Dia's Guia and phone as an option, on by default only when that Dia's Alocação is confirmed (K249, K304).
- **A Guia collecting a payment** sees only the amount to collect, nothing else about the price.
- **The 10% processing on cancellation** is charged on the supplier's penalty (K919), editable.
- The Equipe's own travel (flights, KTX, lodging away from base) is booked as Reservas, and Profissionais who fly have travel data like Viajantes.
- The physical fleet (own and external vehicles) is registered here; the Tabelas de referência only hold their rates.
