...
test("um lead B2C nasce com código, Responsável e prazo de resposta de 2h", async ({ page }) => {
  const codigo = await novaViagem(page, { contato: "Ygara Mendes", telefone: "+55 21 99999-0001" });
  expect(codigo).toBe("V26-0001");
  await expect(page.getByTestId("etapa")).toHaveText("Lead");
  await expect(page.getByText("Responder o primeiro contato — Carlos — prazo 24/09/2026, 11:00")).toBeVisible();
...
test("uma Agência tem prazo de 8h, marca CoreaLux e a cadeia comercial registrada", async ({ page }) => {
...
test("sem resposta humana por 24h, a Viagem é destacada até alguém responder", async ({ page }) => {
  ...
  await expect(page.getByRole("alert")).toHaveText("Sem resposta há mais de 24h");
...
test("trocar o Responsável guarda o histórico e passa a próxima ação", async ({ page }) => {
  ...
  await expect(page.getByText("Carlos: 24/09/2026, 09:00 → 24/09/2026, 10:30")).toBeVisible();
  await expect(page.getByText("Lia: 24/09/2026, 10:30 → agora")).toBeVisible();
  await page.reload();
  await expect(page.getByText("Responder o primeiro contato — Lia")).toBeVisible();
});

test("um contato que não é pedido de viagem é descartado e sai do pipeline", async ({ page }) => {
...
test("o mesmo contato aberto por outra cadeia comercial gera aviso", async ({ page }) => {
  await novaViagem(page, {
    contato: "Isabelle",
  ...
  await novaViagem(page, { contato: "Isabelle", telefone: "11 97777-0003".replace(/^/, "+55 ") });
  await expect(page.getByRole("status")).toContainText("V26-0001 já está aberta com o mesmo contato por outra cadeia comercial");
});

test("Ctrl+K encontra a Viagem pelo telefone e abre", async ({ page }) => {
  await novaViagem(page, { contato: "Fernando", telefone: "+55 11 95555-0004" });
  await page.goto("/");
  await page.keyboard.press("Control+k");
  await page.getByPlaceholder("Código, contato, telefone, e-mail ou agência").fill("95555");
